...
namespace Localfr\AgendizeClientBundle\Service\Agendize\Token;
...
class AccessToken
{
    ...
    public function __construct(array $options = [])
    {
        ...
        if (empty($options['refresh_token'])) {
            throw new InvalidArgumentException('Required option not passed: "refresh_token"');
        }
        ...
        if (isset($options['expire_in'])) {
            if (!is_numeric($options['expire_in'])) {
                throw new \InvalidArgumentException('expire_in value must be an integer');
            }

            $this->expires = $options['expire_in'] != 0 ? $this->getTimeNow() + $options['expire_in'] : 0;
